...
import { methodNotAllowedResponse, parseRequest, problemResponse } from "@/shared/http/problem";
import { jsonResponse } from "@/shared/http/response";
export const runtime = "nodejs"; export const dynamic = "force-dynamic";
export async function GET(request: Request, context: { params: Promise<{ artifactId: string }> }): Promise<Response> { try { await requireServiceToken(request, "admin"); const { artifactId } = await context.params; return jsonResponse(request, artifactSummarySchema.parse(await getArtifactService().get(parseRequest(artifactIdSchema, artifactId)))); } catch (error) { return problemResponse(error, request); } }
