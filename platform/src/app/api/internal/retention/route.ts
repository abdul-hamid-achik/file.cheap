...
export async function GET(request: Request): Promise<Response> { try { await requireServiceToken(request, "cron"); return jsonResponse(request, await getArtifactService().reconcile()); } catch (error) { return problemResponse(error, request); } }
